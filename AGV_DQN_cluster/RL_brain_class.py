...
from MystepFun import AGV_StepFun, AGV_StepFun_check, Train_StepFun
from util import AGV_norm_state, cluster_pattern, best_selection, AGV_Next, AGV_Permit
...
class DeepQNetwork():
    ...
    def choose_action(self, S, IfAssignInit):
        ...
        if IfAssignInit == 1 and S_norm == [0,0,0,0,0,0,0]:
            action_index = random.choice([0, 2])
        return action_index
    ...
    def check_action_AGV_rollout(self, S, check_pt_path, param, Cluster, K, Delta):
        tf.reset_default_graph()
        Problem_state = []
        reach_states = []
        generated_states = []
        ...
        while(len(reach_states) != 0):
            ...
            if S not in generated_states: # if S not tested
                ...
                #original value from trained NN
                pattern_value = self.sess.run(self.q_eval, feed_dict = {self.S: S_norm})
                #print('Original value:', pattern_value, '\n') 
                if self.look_ahead_step == 2:  #ONly lookahead
                    pattern_value, _ = self.Q_value_eval(S, param, "AGV")
                elif self.look_ahead_step == 3: # lookahead + rollout
                    [pattern_value, _] = self.rollout_test(S_norm, S, param, "AGV")  
                #print('Improved value:', pattern_value, '\n')    
                #Cluster module
                if Cluster == 0: #Only the best selection 
                    [all_S_, isDone_test] = best_selection(S, pattern_value, param)
                elif Cluster == 1: #Use cluster algorithm
                    [all_S_, isDone_test] = cluster_pattern(S, param, pattern_value, K, Delta) 
    ...
    def run_AGV(self, S, check_pt_path, param, Cluster, K, Delta, Dead_states):
        tf.reset_default_graph()
        meta_path = check_pt_path + '.meta'
        saver_test = tf.train.import_meta_graph(meta_path)
        saver_test.restore(self.sess, check_pt_path)
        ...
        Nonblocking_states = []
        #Num_Work = np.zeros(3)
        while not isDone_test and step < 100000:    
            if S not in Nonblocking_states:
                Nonblocking_states.append(S)
            print('Current state:', S, '\n')
            #original value from trained NN
            _, S_norm = AGV_norm_state(S)
            pattern_value = self.sess.run(self.q_eval, feed_dict = {self.S: S_norm})
            #print('Original value:', pattern_value, '\n') 
            if self.look_ahead_step == 1:  #ONly lookahead
                pattern_value, _ = self.Q_value_eval(S, param, "AGV", Dead_states)
            elif self.look_ahead_step == 2: # lookahead + rollout
                [pattern_value, _] = self.rollout_test(S_norm, S, param, "AGV", Dead_states)  
            #print('Improved value:', pattern_value, '\n') 
            
            #Cluster module
            if Cluster == 0: #Only the best selection 
                [all_S_, isDone_test] = best_selection(S, pattern_value, param, Dead_states)
            elif Cluster == 1: #Use cluster algorithm
                [all_S_, isDone_test] = cluster_pattern(S, param, pattern_value, K, Delta) 
        ...
        return S, Nonblocking_states, step, isDone_test
    ...
    def check_action_AGV_cluster(self, S, check_pt_path, param, Cluster, K, Delta, Dead_states):  # 240815 by Junjun
        ...
        while(len(reach_states) != 0):
            ...
            if S not in generated_states: # if S not tested
                #print('Current state:', S, '\n')
                _, S_norm = AGV_norm_state(S)  
                pattern_value = np.array(self.sess.run(self.q_eval, feed_dict = {self.S: S_norm}))
                #print(pattern_value)
                #Online improve
                if self.look_ahead_step == 1:  #ONly lookahead
                    pattern_value, _ = self.Q_value_eval(S, param, "AGV", Dead_states)
                ...
                #Cluster
                if Cluster == 0: #Only the best selection 
                    [all_S_, isDone_test, pattern] = best_selection(S, pattern_value, param, Dead_states)
                elif Cluster == 1: #Use cluster algorithm
                    [all_S_, isDone_test] = cluster_pattern(S, param, pattern_value, K, Delta)                                                                                                                                                  
                if isDone_test: #If leads to deadlocks
                    Problem_state.append(S)
                else:
                    for all_s_ in all_S_:  # iterate all next generated states
                        if all_s_ not in reach_states:  # if the newly generated state never appears before
        ...
        return generated_states, Problem_state, reach_states
    
    def Multi_model(self, S, check_pt_path_set, param):
        Problem_state = []
        reach_states = []
        generated_states = []
        # reach_states_full is a 2x list, the first element is the state, the second is the state
        ...
        while (len(reach_states) != 0):  
            ...
            if S not in generated_states: # if S not tested              
                ...
                all_Enb_ = []
                _, S_norm = AGV_norm_state(S)
                for check_pt_path in check_pt_path_set:
                     tf.reset_default_graph()
                     meta_path = check_pt_path + '.meta'
                     saver_test = tf.train.import_meta_graph(meta_path)
                     saver_test.restore(self.sess, check_pt_path)
                     pattern_value = np.array(self.sess.run(self.q_eval, feed_dict = {self.S: S_norm}))
                     #sub-models's decision
                     [_, _, pattern] = best_selection(S, pattern_value, param)
                     #union of all sub-models's decision
                     Pattern = np.union1d(Pattern, pattern).astype(int).tolist()  # 转为列表 + 原生 int
                     
                for event in Pattern:
                     S_temp_ = AGV_Next(S, event, param)
            ...
            if len(generated_states)%100 == 0:
                ...
                print("problem states:", len(Problem_state))
                print("**************************************")
        return generated_states_full, Problem_state, reach_states  

    
    """
    perform rollout based on given state
    """
    def rollout_test(self, S_norm, S, param, case_name, Dead_states):
        ...
        for i in range(self.RO_traces):
            ...
            for RO_i in range(self.RO_depth):                
                if case_name == "AGV":
                    [RO_S_, _, R_t, isDone_test, _, _, _] = AGV_StepFun_check(S_new, act_idx, param)
                elif case_name == "Train":
                    [RO_S_, _, R_t, isDone_test, _, _, _, _] = Train_StepFun(S_new, act_idx, param, 0)
        ...
        if len(top_N_indices) > 0:
            if case_name == "AGV":
                _, pattern_length = self.Q_value_eval(S, param, "AGV", Dead_states)
            elif case_name == "Train":
    ...
    """
    def Q_value_eval(self, S, param, case_name, Dead_states):
        pattern_value = np.zeros(self.n_actions)
        pattern_length = np.zeros(self.n_actions)
        
        for pattern_ind in range(self.n_actions):
            if case_name == "AGV":
                [_, all_S_, R_t, isDone_test, _, _, _] = AGV_StepFun(S, pattern_ind, param)
            elif case_name == "Train":
            ...
            if isDone_test or any(tuple(s) in set(map(tuple, all_S_)) for s in Dead_states):
