...
def AGV_StepFun(obs, event_index, param):
        ...
        [Enb, Enable_P] = AGV_Permit(obs, param) #Modular supervisors permission
        pattern = np.append(param.E_u, param.E_c[event_index]) #{a}\union E_u, where policy=a        
        pattern = np.intersect1d(pattern, Enb)   #The defined event subset   
       
        
        
                   
        # Step
        if len(pattern) == 0:   # undefined controllable events is added
            Enable_P_S_ = []
            obs_ = obs
            all_S_ = [obs]
            stop_ind = 1  #undefined controllable events
            action = -1
        else:                       
            # select action and step
            action = choice(pattern)        
            obs_ = AGV_Next(obs, action, param)         # iterate to the next state
            # iterate all available events in the pattern and calculate the all_S_
            ...
            if any(len(vec) == 0 for vec in all_Enb_): # If any event in the selected pattern leads to a deadlock
                ...
                obs_ = obs
                stop_ind = 2 #deadlocks
                action = -1
            else:
                Enable_P_S_ = [obs_]
            
        # Wrong selection or events
...
# %% step function for AGV case study
def AGV_StepFun_check(obs, event_index, param):    
    isDone = 0
    IfAppearGoodEvent = 0
    stop_ind = 0
    all_S_ = []
    ...
    [Enb, Enable_P] = AGV_Permit(obs, param) #Modular supervisors permission
    if param.E_c[event_index] not in Enb:
        pattern = Enb
    else:
        pattern = np.append(param.E_u, param.E_c[event_index]) #{a}\union E_u, where policy=a        
        pattern = np.intersect1d(pattern, Enb)   #The defined event subset   
